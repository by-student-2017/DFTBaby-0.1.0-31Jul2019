...
class MoleculeObject:
    ...
    def _create_visualization(self, atomlist):
        ...
        tube = mlab.pipeline.tube(bonds, tube_radius=0.05,
                                  figure=self.scene.mayavi_scene)
...
class ChargesObject:
    ...
    def _create_visualization(self, atomlist, charges, fragments):
        ...
        if self.show_flags["screening charges (COSMO)"] == True:
            cavity = ImplicitSolvent.SolventCavity()
...
class Visualization(HasTraits):
    ...
    def plotCube(self, cube, iso=0.04):
        ...
        if self.show_flags["isosurface"] == True:
            # red position lobe
            positive_lobe = mlab.contour3d(x,y,z,cube.data, contours=[+iso], color=(1.0, 0.0, 0.0), figure=self.scene.mayavi_scene)
            # blue negative lobe
            negative_lobe = mlab.contour3d(x,y,z,cube.data, contours=[-iso], color=(0.0, 0.0, 1.0), figure=self.scene.mayavi_scene)
            #
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print "Usage: %s <list of cube- or xyz-files>" % sys.argv[0]
