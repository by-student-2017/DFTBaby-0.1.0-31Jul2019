#!/usr/bin/env python
"""
computes the photoelectron angular distribution (PAD) for ionization of H2+ using the numerically
exact bound and continuum orbitals of this single-electron Hamiltonian.

The curves for sigma(PKE) and beta(PKE) should be compared with Fig. 3 of Ref. [1] or table 4 of Ref. [2]

References:
-----------
[1] Rei Matsuzaki, Satoshi Yabushita
    "Calculation of Photoionization Differential Cross Sections Using Complex Gauss-Type Orbitals"
    Journal of Computational Chemistry 2017, 38, 2030-2040
[2] M. Brosolo, P. Decleva,
    "Variational approach to continuum orbitals in a spline basis: an application to H2+ photoionization",
    Chem. Phys. 159, (1992), 185-196
"""
...
from DFTB.Scattering.hydrogen_molecular_ion import DimerWavefunctions, SeparationConstants
from DFTB.Scattering.VariationalKohn import transition_dipole_integrals, orientation_averaged_pad

class WrapperWavefunction:
    """
    this wrapper functions makes the wavefunctions of H2+ have the same interface
    as used in the module VariationalKohn
    """
    def __init__(self, wavefunction):
        self.wavefunction = wavefunction
    def amp(self, x,y,z):
        return self.wavefunction((x,y,z), None)
...
def photoangular_distribution(Dmo, Amo, LMs, energy):
    ...
    assert abs(sigma - sigma_oscsum) < 1.0e-4
    
    return sigma.real, beta.real

def h2_ion_averaged_pad_scan(energy_range, data_file,
    ...
    Cube.function_to_cubefile(atomlist, wavefunction1, filename="/tmp/h2+_bound_orbital_%d_%d_%d.cube" % (m,n,q), dbuff=10.0)
    ###
    ...
    for i,energy in enumerate(energy_range):
        if i % size == rank:
            ...
            phase_shifts = []
            # transition dipoles
            nc = 2*(Lsep.mmax+1)*(Lsep.nmax+1)-1  # number of continuum orbitals at each energy
            ...
            for m in range(0, Lsep.mmax+1):
                ...
                for trig in Psolutions:
                    for n in range(0, Lsep.nmax+1):
                        Delta, (Rfunc2,Sfunc2,Pfunc2),wavefunction2 = wfn.getContinuumOrbital(m,n,trig,energy)
                        #print "energy= %s eV   m= %d n= %d  phase shift= %s \pi rad" % (energy*AtomicData.hartree_to_eV, m,n,Delta / np.pi)
                        print "      %3.1d             %2.1d        %2.1d                  %s                         %8.6f" % (ic,m,n,trig,Delta)
                        ...
                        Cube.function_to_cubefile(atomlist, wavefunction2, filename="/tmp/h2+_continuum_orbital_%d_%d_%s.cube" % (m,n,trig), dbuff=10.0)
                        ###
                        ...
                        dipoles_RSP[0,ic,:] = wfn.transition_dipoles(Rfunc1,Sfunc1,Pfunc1,
                                                                     Rfunc2,Sfunc2,Pfunc2)
                        ic += 1
            ...
            for m in range(0, Lsep.mmax+1):
                ...
                for trig in Psolutions:
                    for n in range(0, Lsep.nmax+1):
                        print "continuum orbital %d  (m= %d  n= %d  trig= %s)" % (ic, m, n, trig)
                        print " from factorization R*S*P :  %s" % dipoles_RSP[0,ic,:]
                        print " Becke's integration      :  %s" % dipoles[0,ic,:]
...
if __name__ == "__main__":
    ...
    compute the photoangular distribution (PAD) for ionization from the 1\sigma_g orbital of H2+
    to the continuum. The bond length of the hydrogen molecular ion is set to 2.0 bohr.
    ...
    print "computing photoangular distribution for orbital 1\sigma_g"
    h2_ion_averaged_pad_scan(energy_range, data_file, 
